...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val wordCharacters:List[Char] = w.toLowerCase.toList
    wordCharacters.groupBy(identity).map(x => (x._1, x._2.length)).toList.sortBy(x => x._1)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def iter(occ: Occurrences, acc: List[Occurrences]): List[Occurrences] = {
      if(occ.isEmpty) acc
      else {
        val curr_occ = occ.head
        val curr_char = curr_occ._1
        val tuplesToBeAdded = for(i <- 1 to curr_occ._2) yield (curr_char, i)
        val newAcc = addTupples(tuplesToBeAdded.toList, acc)
        iter(occ.tail, newAcc)
      }
    }
