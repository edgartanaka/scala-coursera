...
class TweetSetSuite {
  trait TestSets {
    ...
    // large sets
    var large1:TweetSet = new Empty
    var large2:TweetSet = new Empty
    val r = new scala.util.Random(31)
    val largeSize = 5
    for(i <- 1 to largeSize) {
      large1 = large1.incl(new Tweet("a" + i, "a" + i,20))
      large2 = large2.incl(new Tweet("b" + i,"b" + i, 20))
    }
  ...
  @Test def `union: large`: Unit =
    new TestSets {
      println("------- large sets test started -------")
      println(large1)
      println(large2)
      val unioned = large1.union(large2)
      assertEquals(10, size(unioned))
      println(asSet(unioned))
    }
