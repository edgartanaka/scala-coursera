...
class Empty extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    throw new java.util.NoSuchElementException("most retweeted of empty element is invalid")
  }
...
//    println("union(): on this[" + this.toString + "] and that[" + that + "]")
...
//  }
  ...
  override def mostRetweeted: Tweet = {
    val bestLeft = if(!left.isEmpty()) left.mostRetweeted else elem
    val bestRight = if(!right.isEmpty()) right.mostRetweeted else elem
    val bestChildren = if(bestLeft.retweets > bestRight.retweets) bestLeft else bestRight
    val bestOfAll = if(elem.retweets > bestChildren.retweets) elem else bestChildren
    bestOfAll
  }

  override def descendingByRetweet: TweetList = {
    val mostRetweeted = this.mostRetweeted
    val newTree = this.remove(mostRetweeted)
    new Cons(mostRetweeted, newTree.descendingByRetweet)
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(
    x => google.exists(x.text.contains))

  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(
    x => apple.exists(x.text.contains))
